		...
		if ($column->type === 'boolean') {
			...
			if ($column->type !== 'string' || $column->size === null) {
				if($column->dbType == 'date') {
					return "\$form->datePickerGroup(\$model,'{$column->name}',array('widgetOptions'=>array('options'=>array(),'htmlOptions'=>array()), 'prepend'=>'<i class=\"bi bi-calendar\"></i>', 'append'=>'Click on Month/Year to select a different Month/Year.'))";
				} else {
				...
				if (strpos ( $column->dbType, 'enum(' ) !== false) {
					...
					$dropdown_options .= ")";
					return "\$form->dropDownListGroup(\$model,'{$column->name}', array('widgetOptions'=>array('data'=>{$dropdown_options}, 'htmlOptions'=>array())))";
				} else {
